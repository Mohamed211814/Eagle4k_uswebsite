...
export const Pricing: React.FC<PricingProps> = ({ onOpenCheckout }) => {
  const [activeTab, setActiveTab] = useState<'subscriptions' | 'resellers'>('subscriptions');
  ...
  return (
    <section id="pricing" className="py-20 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight">
            {activeTab === 'subscriptions' ? (
              ...
              <>Eagle4k <span className="text-gradient-gold">Reseller IPTV Panels</span></>
            )}
          </h2>
          <p className="text-slate-400 text-base sm:text-lg">
            {activeTab === 'subscriptions'
              ? 'No long-term contracts. Cancel anytime. Instant email delivery with Xtream API credentials & M3U playlist.'
              : 'Become an official Eagle4k reseller. Manage accounts, create free trials, and generate instant revenue with lifetime credits.'}
          </p>
        </div>

        {/* Tab Switcher: Subscriptions vs Panel Resellers */}
        <div className="mt-8 flex justify-center">
          <div className="glass-panel p-1.5 rounded-2xl flex items-center gap-2 border border-slate-800">
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-xs font-extrabold transition-all cursor-pointer flex items-center gap-1.5 ${
                activeTab === 'resellers'
                  ? 'bg-amber-500 text-slate-950 shadow-md'
                  : 'text-slate-400 hover:text-white'
              }`}
        ...
        {/* Pricing Cards Grid - Standard Subscriptions */}
        {activeTab === 'subscriptions' ? (
          ...
          /* Reseller Panels Display (2 Packs: 240 credits / 10y & 480 credits / 20y cheaper) */
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto items-stretch">
            {RESELLER_PLANS.map((rPlan) => {
              const isPopular = rPlan.popular;
              return (
                <div
                  key={rPlan.id}
                  className={`relative rounded-3xl transition-all duration-300 flex flex-col justify-between p-8 ${
                    isPopular
                      ? 'glass-panel-gold border-2 border-amber-500 shadow-2xl shadow-amber-500/25 scale-105 z-20'
                      : 'glass-panel border border-slate-800 hover:border-slate-700'
                  }`}
                >
                  {/* Popular / Bulk Discount Badge */}
                  {isPopular && (
                  ...
                  <div>
                    <div className="flex items-center justify-between">
                      <h3 className="text-xl font-black text-white">{rPlan.name}</h3>
                      {rPlan.discountBadge && (
                        <span className="px-3 py-1 rounded-full text-[11px] font-black bg-amber-500/20 text-amber-400 border border-amber-500/40">
                          {rPlan.discountBadge}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-4 flex items-baseline gap-2">
                      <span className="text-5xl font-black text-white">{rPlan.price}</span>
                      <span className="text-xs text-amber-400 font-bold">({rPlan.credits} Credits)</span>
                    </div>

                    <div className="mt-2 space-y-1">
                      <div className="text-sm font-extrabold text-white bg-slate-900/90 border border-slate-800 px-3 py-1.5 rounded-lg inline-block">
                        ⚡ {rPlan.yearsEquivalent}
                      </div>
                      <p className="text-xs text-emerald-400 font-semibold pt-1">
                        {rPlan.perCreditPrice}
                      </p>
                    </div>

                    {/* Reseller Feature Checklist */}
                    <div className="mt-6 space-y-3 pt-6 border-t border-slate-800">
                      {rPlan.features.map((feature, i) => (
                  ...
                  <div className="mt-8">
                    <button
                      onClick={() => onOpenCheckout(rPlan.id)}
                      className={`w-full py-4 rounded-xl font-black text-xs transition-all shadow-lg cursor-pointer flex items-center justify-center gap-2 ${
                        isPopular
                          ? 'bg-amber-400 hover:bg-amber-300 text-slate-950 shadow-amber-500/30'
                          : 'bg-slate-900 hover:bg-slate-800 text-white border border-slate-700 hover:border-amber-500/50'
                      }`}
        ...
        {/* Guarantee & Reseller Support Banner */}
        <div className="mt-16 text-center max-w-xl mx-auto p-4 rounded-2xl glass-panel border border-slate-800 flex items-center justify-center gap-3">
          ...
          <p className="text-xs text-slate-300 text-left">
            <strong className="text-white">Instant Reseller Panel Setup</strong>. Credits never expire. Full Xtream UI access details sent to your email instantly after ordering.
          </p>
        </div>
